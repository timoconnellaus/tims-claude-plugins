---
topic: tanstack-router/framework/solid/guide/path-params
title: Path Params
description: "Path params are used to match a single segment (the text until the
  next `/`) and provide its value back to you as a **named** variable. They are
  defined by using the `$` character prefix in the path, "
version: latest
sourceUrl: https://github.com/tanstack/router/blob/main/docs/router/framework/solid/guide/path-params.md
tags:
  - tanstack-router
  - framework
---
