---
topic: tanstack-router/framework/react/api/router/notfoundfunction
title: NotFoundFunction
description: The `notFound` function returns a new `NotFoundError` object that
  can be either returned or thrown from places like a Route's `beforeLoad` or
  `loader` callbacks to trigger the `notFoundComponent`.
version: latest
sourceUrl: https://github.com/tanstack/router/blob/main/docs/router/framework/react/api/router/notFoundFunction.md
tags:
  - tanstack-router
  - framework
---
